...
#frontend
docker run -dit --name ${frontend_con_name} -p ${frontend_port}:${frontend_port} -e frontend_port=${frontend_port} -e backend_con_name=${backend_con_name} -e custom_network_name=${custom_network_name} -e backend_port=${backend_port} --net ${custom_network_name} ${frontend_image} /bin/bash
#backend
docker run -dit --name ${backend_con_name} -p ${backend_port}:${backend_port} -e backend_port=${backend_port} --net ${custom_network_name} ${backend_image} /bin/bash
...
#redis
docker run -dit --name ${redis_con_name} --net ${custom_network_name} ${redis_image}
